...
fn build_bvh(triangles: &[Triangle]) -> Bvh<Triangle> {
    ...
    fn build_recursive(triangles: &mut [Triangle]) -> Bvh<Triangle> {
        if triangles.len() <= 4 {
...
fn find_split(triangles: &[Triangle]) -> (usize, f32) {
    ...
    let mut min_right = calculate_aabb(&triangles[0..1]).expand(&calculate_aabb(&triangles[1..]));
    ...
    for (i, triangle) in triangles.iter().enumerate().skip(1) {
        ...
        let overlap = left_aabb.overlap(&right_aabb);
